...
import {
  ZoomIn, ZoomOut, Home, RotateCw, RotateCcw,
  Download, ArrowLeft, Grid3X3, X, RefreshCcw,
  Expand, Camera, MoreHorizontal, Square,
  Circle as CircleIcon, MousePointer2, ChevronDown,
  Layers, FileImage, AlertCircle, Loader2
} from "lucide-react";
import { useNavigate } from "react-router-dom";
...
const WBC_COLORS = {
  'neutrophil': '#ef4444',            // Red
  'lymphocyte': '#3b82f6',            // Blue
  'monocyte': '#22c55e',              // Green
  'eosinophil': '#f97316',            // Orange
  'basophil': '#a855f7',              // Purple
  'blast_cells': '#ec4899',           // Pink
  'immature_granulocytes': '#eab308'  // Yellow
};
...
const SlideSelector = ({ slides, selectedSlide, onSelect, loading }) => {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center gap-2 px-4 py-2 rounded-xl border border-border bg-card hover:bg-muted/50 transition-colors text-sm font-medium min-w-[220px] justify-between shadow-sm"
      >
        <div className="flex items-center gap-2">
          {loading ? <Loader2 className="w-4 h-4 animate-spin text-primary" /> : <Layers className="w-4 h-4 text-primary" />}
          <span className="truncate">{selectedSlide ? selectedSlide.name : "Select a slide…"}</span>
        </div>
        <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform ${open ? "rotate-180" : ""}`} />
      </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -8 }}
            transition={{ duration: 0.15 }}
            className="absolute top-full mt-2 left-0 w-full bg-card border border-border rounded-xl shadow-xl z-50 overflow-hidden max-h-96 overflow-y-auto"
          >
            {slides.length === 0 ? (
              <div className="px-4 py-3 text-sm text-muted-foreground">No slides found</div>
            ) : (
              slides.filter((slide) => slide.has_dzi).map((slide) => (
                <button
                  key={slide.name}
                  onClick={() => { onSelect(slide); setOpen(false); }}
                  className={`w-full flex items-center gap-3 px-4 py-3 text-sm hover:bg-muted/50 transition-colors text-left ${selectedSlide?.name === slide.name ? "bg-primary/10 text-primary" : ""}`}
                >
                  <FileImage className="w-4 h-4 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="font-medium truncate">{slide.name}</div>
                  </div>
                  {selectedSlide?.name === slide.name && <div className="w-2 h-2 rounded-full bg-primary shrink-0" />}
                </button>
              ))
            )}
          </motion.div>
...
const Viewer = () => {
  ...
  // ── Slide & Metadata State ──
  const [slides, setSlides] = useState([]);
  const [slidesLoading, setSlidesLoading] = useState(true);
  const [selectedSlide, setSelectedSlide] = useState(null);
  const [metadata, setMetadata] = useState(null); // AI JSON Data
  const [viewMode, setViewMode] = useState("dzi");
  const [error, setError] = useState(null);
  ...
  const [isPatchFullscreen, setIsPatchFullscreen] = useState(false);
  const [fullPatchZoom, setFullPatchZoom] = useState([100]);
  const [showWbcBoxes, setShowWbcBoxes] = useState(true);
  ...
  const handleSlideSelect = (slide) => {
    ...
    if (slide.has_tiles) {
      setTilesLoading(true);
      // Fetch Tiles Manifest
      authFetch(`${API_BASE}/api/slides/${slide.name}/tiles`)
        .then((r) => r.json())
        .then((data) => setTileGrid(data))
        .catch(() => setError("Failed to load tile list"))
        .finally(() => setTilesLoading(false));
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (!selectedPatch || !tileGrid) return;
      const { row, col } = selectedPatch;
      const rows = tileGrid.rows || 4;
      const cols = tileGrid.cols || 4;
      let newRow = row;
      let newCol = col;
      ...
      const nextTile = tileGrid.tiles.find(t => t.row === newRow && t.col === newCol);
      if (nextTile) handleGridClick(nextTile);
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [selectedPatch, tileGrid]);

  // ── Web-Safe Tile Export ──
  const handleExportTile = () => {
    if (!selectedPatch?.url) return;
    const a = document.createElement("a");
    a.href = selectedPatch.url;
    a.download = selectedPatch.filename || `patch_R${selectedPatch.row}_C${selectedPatch.col}.jpg`;
  ...
  const handleGridClick = useCallback(async (tile) => {
    ...
    try {
      ...
      setSelectedPatch(prev => {
        ...
        return { row: tile.row, col: tile.col, url: objectUrl, filename: tile.filename };
      });
    } catch (err) {
      console.error(err);
      setSelectedPatch({ row: tile.row, col: tile.col, url: null, filename: tile.filename });
  ...
  useEffect(() => {
    ...
    authFetch(dziUrl)
      .then(response => {
        if (!response.ok) throw new Error("Failed to fetch DZI");
        return response.text();
      })
      .then(dziXmlString => {
        const parser = new DOMParser();
        ...
        const dziObject = {
          Image: {
            ...
            Overlap: imageNode.getAttribute("Overlap"),
            TileSize: imageNode.getAttribute("TileSize"),
            Size: { Height: sizeNode.getAttribute("Height"), Width: sizeNode.getAttribute("Width") }
          }
        };
  ...
  // AI Details calculation for the current active patch
  const activeTileWbcs = selectedPatch && metadata?.tiles ? (metadata.tiles[selectedPatch.filename] || []) : [];
  const activeTileCounts = activeTileWbcs.reduce((acc, curr) => {
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      >
        <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-4 flex-wrap">
          ...
          {/* Conditional Dropdown / Exit Button */}
          <div className="flex items-center gap-3 flex-wrap">
            {selectedPatch ? (
              <Button 
                variant="outline" 
              ...
              <SlideSelector slides={slides} selectedSlide={selectedSlide} onSelect={handleSlideSelect} loading={slidesLoading} />
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            {!isPatchFullscreen && selectedSlide?.has_tiles && (
              <Button variant={showGrid ? "default" : "outline"} size="sm" onClick={() => setShowGrid(!showGrid)}>
                <Grid3X3 className="w-4 h-4 mr-2" /> {showGrid ? "Hide Grid" : "Show Grid"}
              </Button>
            )}
            <Button variant="outline" size="sm" onClick={handleExportTile} disabled={!selectedPatch}>
              <Download className="w-4 h-4 mr-2" /> Export
            </Button>
          </div>
        </div>
      ...
      <div className="flex-1 container mx-auto px-4 py-6 overflow-hidden">
        <AnimatePresence>
          {error && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mb-4 flex items-center gap-3 bg-destructive/10 border border-destructive/20 text-destructive rounded-xl px-4 py-3 text-sm">
        ...
        {selectedSlide && (
          <div className="grid lg:grid-cols-[1fr_300px] gap-6 h-full">
            ...
            <motion.div className="bg-muted/30 rounded-2xl border border-border shadow-card overflow-hidden relative group" style={{ height: "calc(100vh - 160px)" }}>
              ...
              {!isPatchFullscreen && (
                <div className="absolute left-4 top-1/2 -translate-y-1/2 z-20 flex flex-col bg-white/90 backdrop-blur-sm rounded-full shadow-lg border border-border/50">
                  ...
                  <motion.div initial={false} animate={{ height: isToolbarExpanded ? "auto" : 0, opacity: isToolbarExpanded ? 1 : 0 }} className="flex flex-col items-center overflow-hidden">
                    <div className="flex flex-col gap-2 pb-2 px-2">
                      <div className="h-px w-6 bg-border mx-auto my-1" />
                      <Button variant={drawingMode === "select" ? "default" : "ghost"} size="icon" className="rounded-full h-10 w-10" onClick={() => handleModeChange("select")}><MousePointer2 className="w-5 h-5" /></Button>
                      <Button variant={drawingMode === "rect"   ? "default" : "ghost"} size="icon" className="rounded-full h-10 w-10" onClick={() => handleModeChange("rect")}><Square className="w-5 h-5" /></Button>
                      <Button variant={drawingMode === "circle"  ? "default" : "ghost"} size="icon" className="rounded-full h-10 w-10" onClick={() => handleModeChange("circle")}><CircleIcon className="w-5 h-5" /></Button>
                      <div className="h-px w-6 bg-border mx-auto my-1" />
                      <Button variant="ghost" size="icon" className="rounded-full h-10 w-10 hover:bg-primary/10" onClick={handleReset}><Home className="w-5 h-5 text-primary" /></Button>
                      <Button variant="ghost" size="icon" className="rounded-full h-10 w-10 hover:bg-primary/10" onClick={handleZoomIn}><ZoomIn className="w-5 h-5" /></Button>
                      <Button variant="ghost" size="icon" className="rounded-full h-10 w-10 hover:bg-primary/10" onClick={handleZoomOut}><ZoomOut className="w-5 h-5" /></Button>
                    ...
                    <>
                      <div className="absolute inset-0 grid" style={{ gridTemplateColumns: `repeat(${gridCols}, 1fr)`, gridTemplateRows: `repeat(${gridRows}, 1fr)` }}>
                        {tileGrid.tiles.map((tile) => {
                          const isSelected = selectedPatch?.row === tile.row && selectedPatch?.col === tile.col;
                          return (
                            <div key={`tile-${tile.row}-${tile.col}`} className={`relative overflow-hidden border border-white/10 cursor-pointer transition-all hover:brightness-110 hover:z-10 ${isSelected ? "ring-2 ring-primary ring-inset" : ""}`} onClick={() => handleGridClick(tile)}>
                              <img src={`${API_BASE}/api/slides/${selectedSlide.name}/tiles/${tile.filename}`} alt="Tile" className="w-full h-full object-cover" loading="lazy" onError={(e) => { e.currentTarget.style.background = "#334155"; }} />
                              <div className="absolute bottom-1 right-1 bg-black/50 text-white text-[10px] px-1 rounded">{tile.row},{tile.col}</div>
                            </div>
                          );
                        })}
                      </div>
                      {showGrid && (
                        <div className="absolute inset-0 grid pointer-events-none" style={{ gridTemplateColumns: `repeat(${gridCols}, 1fr)`, gridTemplateRows: `repeat(${gridRows}, 1fr)`, zIndex: 10 }}>
                          {Array.from({ length: gridRows * gridCols }).map((_, i) => (
                            <div key={i} className="border border-primary/30 flex items-center justify-center">
                              <span className="text-[10px] text-white/50 font-mono">{Math.floor(i / gridCols)},{i % gridCols}</span>
                            </div>
                          ))}
                        </div>
                      )}
            ...
            <motion.div className="space-y-4 overflow-y-auto h-[calc(100vh-160px)] pr-2">
              ...
              {/* WBC Overview Panel (Shows if metadata exists) */}
              {!selectedPatch && !showGrid && metadata?.class_counts && (
                <div className="bg-card border border-border rounded-2xl p-6 shadow-card flex flex-col gap-4">
                  <div className="flex items-center justify-between border-b border-border pb-3">
                    <h2 className="text-[15px] font-extrabold text-foreground tracking-tight">WBC Overview</h2>
                    <span className="bg-primary/10 text-primary font-black text-xs px-2.5 py-1 rounded border border-primary/20 shadow-sm uppercase tracking-widest">
                      {metadata.total_wbcs} Total
                    </span>
                  </div>
                  <div className="flex flex-col gap-2">
                    {Object.entries(metadata.class_counts).map(([cName, count]) => (
                      <div key={cName} className="flex items-center justify-between p-3 bg-muted/50 rounded-xl border border-border transition hover:bg-muted">
                        <span className="text-sm font-bold text-foreground capitalize">{cName.replace('_', ' ')}</span>
                        <span className="font-mono text-xs font-bold text-muted-foreground bg-background px-2 py-1 rounded border border-border shadow-sm">{String(count)}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <AnimatePresence>
                {selectedPatch && !isPatchFullscreen && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="bg-primary/5 border border-primary/20 rounded-2xl p-4 shadow-sm">
                    <div className="flex justify-between items-center mb-2">
                      <h3 className="font-semibold text-primary">Selected Patch</h3>
                      <div className="flex gap-1">
                    ...
                    <div className="mt-4 flex flex-col gap-2">
                      <Button onClick={() => { setIsPatchFullscreen(true); setShowMetadataPanel(true); }} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold shadow-md">Analyze WBCs in Patch</Button>
                      <Button onClick={handleExportTile} variant="outline" className="w-full">Export Tile JPG</Button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              {!selectedPatch && selectedSlide && (
                <>
                  <div className="bg-card border border-border rounded-2xl p-4 shadow-card">
                    ...
                    <div className="space-y-1.5 text-xs text-muted-foreground">
                      <div className="flex justify-between"><span>Name</span><span className="text-foreground font-medium truncate ml-2 max-w-[140px]">{selectedSlide.name}</span></div>
                      <div className="flex justify-between"><span>AI Analysis</span><span className={metadata ? "text-primary font-bold" : "text-muted-foreground"}>{metadata ? "✓ Complete" : "—"}</span></div>
                    </div>
                  </div>
                  <div className="bg-card border border-border rounded-2xl p-6 shadow-card">
                    <h3 className="font-semibold mb-4">Slide Controls</h3>
                    <div className="space-y-4">
                      <div className="flex items-center gap-2">
                        <Button variant="outline" size="icon" onClick={handleZoomOut}><ZoomOut className="w-4 h-4" /></Button>
                        <Slider value={zoom} onValueChange={handleSliderZoom} min={25} max={400} step={25} className="flex-1" />
                        <Button variant="outline" size="icon" onClick={handleZoomIn}><ZoomIn className="w-4 h-4" /></Button>
                      </div>
                    </div>
                  </div>
